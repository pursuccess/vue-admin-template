  ...
  <div class="customer-container">
    ...
    <div class="SearchBasic">
            ...
            <div>
              <el-form :model="info" label-position="right" label-width="130px" size="small">
                <el-form-item label="注册时间">
                  <el-row>
                    <el-col :span="11">
                      <el-date-picker v-model="info.registerTimeMin" type="date" placeholder="请选择" value-format="yyyy-MM-dd" />
                    </el-col>
                    <el-col :span="2" class="line">-</el-col>
                    <el-col :span="11">
                      <el-date-picker v-model="info.registerTimeMax" type="date" placeholder="请选择" value-format="yyyy-MM-dd" />
                    </el-col>
                  </el-row>
                </el-form-item>
                <el-form-item label="最近登录时间">
                  <el-row>
                    <el-col :span="11">
                      <el-date-picker v-model="info.loginTimeMin" type="date" placeholder="请选择" value-format="yyyy-MM-dd" />
                    </el-col>
                    <el-col :span="2" class="line">-</el-col>
                    <el-col :span="11">
                      <el-date-picker v-model="info.loginTimeMax" type="date" placeholder="请选择" value-format="yyyy-MM-dd" />
                    </el-col>
                  </el-row>
                </el-form-item>
                ...
                <el-form-item label="客户绑定店铺近3个月每月销售金额（单位：万元）" class="sales">
                  <el-form-item>
                    <el-select v-model="salesRangeValue" placeholder="快捷选择" clearable @change="salesRangeChange">
            ...
            <div>
              <el-form :model="info" label-position="right" label-width="120px" size="small">
                ...
                <el-form-item label="上次跟进时间">
                  <el-row>
                    <el-col :span="11">
                      <el-date-picker v-model="info.followTimeMin" type="date" placeholder="请选择" value-format="yyyy-MM-dd" />
                    </el-col>
                    <el-col :span="2" class="line">-</el-col>
                    <el-col :span="11">
                      <el-date-picker v-model="info.followTimeMax" type="date" placeholder="请选择" value-format="yyyy-MM-dd" />
                    </el-col>
                  </el-row>
                </el-form-item>
                <el-form-item label="上次购买时间">
                  <el-row>
                    <el-col :span="11">
                      <el-date-picker v-model="info.orderTimeMin" type="date" placeholder="请选择" value-format="yyyy-MM-dd" />
                    </el-col>
                    <el-col :span="2" class="line">-</el-col>
                    <el-col :span="11">
                      <el-date-picker v-model="info.orderTimeMax" type="date" placeholder="请选择" value-format="yyyy-MM-dd" />
                    </el-col>
                  </el-row>
                </el-form-item>
                <el-form-item label="客户分配时间">
                  <el-row>
                    <el-col :span="11">
                      <el-date-picker v-model="info.distributeTimeMin" type="date" placeholder="请选择" value-format="yyyy-MM-dd" />
                    </el-col>
                    <el-col :span="2" class="line">-</el-col>
                    <el-col :span="11">
                      <el-date-picker v-model="info.distributeTimeMax" type="date" placeholder="请选择" value-format="yyyy-MM-dd" />
                    </el-col>
                  </el-row>
                </el-form-item>
              </el-form>
      ...
      <div class="text-center my-3">
        <el-button class="filter-item" type="primary" style="width: 150px;" @click="handleFilter">筛选</el-button>
      </div>
    </div>
    ...
    <div class="table-container">
      <div class="my-3 filter-container">
        <el-row>
          <el-col :span="12">
            <b style="line-height: 40px; font-size: 18px;">总数：{{ total }}</b>
          </el-col>
          <el-col :span="12" class="flex justify-content-right">
      ...
      >
        <el-table-column v-if="!adminId" align="center" label="标签" width="100">
          <template slot-scope="{row}">
            <el-tag v-if="row.tags[0]" size="small">{{ row.tags[0] }}</el-tag>
            <el-tag v-if="row.tags[1]" size="small">{{ row.tags[1] }}</el-tag>
            <div v-if="row.tags.length>2">
              <span v-if="!isPublic" style="color:#409EFF;cursor:pointer" @click="handleDetailTag(row)">....</span>
        ...
        <el-table-column align="center" label="客户名称" width="150">
          <template slot-scope="{row}">
            <div style="white-space:nowrap;">
              <img v-if="row.avatar" :src="row.avatar" class="customer-avatar">
        ...
        <el-table-column align="center" width="110" label="客户来源">
          <template slot-scope="scope">
        ...
        <el-table-column v-if="isPublic" class-name="status-col" label="操作" align="center" width="110">
          <template slot-scope="{row}">
            <el-button v-if="pick_quota_remain" size="mini" @click="handleGet(row)">领取</el-button>
          </template>
        </el-table-column>
        <el-table-column v-else-if="!isPublic && !adminId" class-name="status-col" label="操作" align="left" width="330">
          <template slot-scope="{row}">
            ...
            <el-button v-if="!row.is_delay" size="mini" @click="handleDelay(row)">延期</el-button>
          </template>
        </el-table-column>
        <el-table-column v-if="teacherEditAuth" class-name="status-col" label="导师客服设置" align="center" width="110">
          <template slot-scope="{row}">
            <el-button size="mini" @click="handleTeacher(row)">编辑</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog :visible.sync="dialogFormVisible" :close-on-click-modal="false" width="60%" custom-class="customerDialog" :modal-append-to-body="false">
      <div slot="title" style="font-size:20px">
        {{ dialogTitle }} <span style="font-size: 14px;color:#999;margin:0 10px;">ID:{{ rowId }}</span> <el-button v-if="!isPublic && !adminId" size="mini" @click="handleTag">添加标签</el-button>
      </div>
      <tab :temp="temp" />
    </el-dialog>
    ...
    >
      <tag ref="tag" :dynamic-tags="dynamicTags" />
      <div slot="footer">
    ...
    <el-dialog title="添加跟进记录" :visible.sync="dialogFollowFormVisible" :close-on-click-modal="false" width="500px">
      <el-form ref="followForm" :model="followForm" style="padding-left: 40px" label-width="100px">
        ...
        <el-form-item label="客户反馈问题" prop="feedback">
          <el-input v-model="followForm.feedback" type="textarea" placeholder="请输入客户反馈问题" />
        </el-form-item>
        <el-form-item label="详细跟进情况" prop="detail">
          <el-input v-model="followForm.detail" type="textarea" placeholder="请输入详细跟进情况" />
        </el-form-item>
    ...
    <el-dialog title="新增订单" :visible.sync="dialogAddFormVisible" :close-on-click-modal="false" width="500px">
      <el-form ref="addForm" :model="addForm" style="padding-left: 40px" label-width="100px">
        ...
        <el-form-item label="上传凭证" prop="image" :rules="{required: true, message: '请上传凭证'}">
          <el-upload
            class="uploader-container"
            :action="uploadUrl"
            :headers="uploadHeaders"
    ...
    <el-dialog title="编辑导师客服" :visible.sync="dialogTeacherFormVisible" :close-on-click-modal="false" width="400px" :modal-append-to-body="false">
      <el-form ref="teacherForm" :model="teacherForm" style="padding-left: 40px" label-width="80px">
        <el-form-item label="导师" prop="teacher" :rules="{required: true, message: '请指定导师'}">
          <el-select v-model="teacherForm.teacher" placeholder="请选择">
            <el-option v-for="(item, key) in teacherList" :key="key" :label="item.name" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item label="客服" prop="customerService" :rules="{required: true, message: '请指定客服'}">
          <el-select v-model="teacherForm.customerService" placeholder="请选择">
            <el-option v-for="(item, key) in customerServiceList" :key="key" :label="item.name" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item>
          <el-button @click="dialogTeacherFormVisible = false; resetForm('teacherForm')">取 消</el-button>
          <el-button type="primary" @click="teacherFormSubmit">确 定</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
  ...
  methods: {
    fetchSellers() {
      getCustomerSellers().then(response => {
        ...
        this.teacherList = response.data.sellers
        this.customerServiceList = response.data.sellers
      })
    },
    fetchData() {
      this.listLoading = true
      const query = Object.assign({}, this.listQuery, this.info, { adminId: this.adminId })
      if (this.isPublic) {
    ...
    salesRangeChange() {
      if (isNull(this.salesRangeValue)) {
        ...
        const rangeValue = JSON.parse(this.salesRangeValue)
    ...
    tagSave() {
      const query = { userId: this.rowId, tags: this.dynamicTags }
    ...
    followFormSubmit() {
      const query = { userId: this.rowId }
      for (const key of Object.keys(this.followForm)) {
    ...
    transformSubmit() {
      const query = { userId: this.rowId, seller: this.transform.seller }
    ...
    addFormSubmit() {
      const query = { userId: this.rowId }
      for (const key of Object.keys(this.addForm)) {
    ...
    handleTeacher(row) {
      ...
      this.rowId = row.userId
      this.teacherForm.teacher = row.teacher
      this.teacherForm.customerService = row.customerService
    },
    teacherFormSubmit() {
      const query = { userId: this.rowId, teacher: this.teacherForm.teacher, customerService: this.teacherForm.customerService }
      this.$refs['teacherForm'].validate((valid) => {
        if (valid) {
          customerTeacher(query).then(response => {
            this.$message({
              type: 'success',
              message: '导师客服设置成功!'
            })
